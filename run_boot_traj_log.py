...
def run_bootstrap_trajectory(base_seed, traj_id, boot_id, X, y, alphas, reg, beta_a, beta_b):
    N, p = X.shape
...
def run_batch_trajectories(base_seed, batch_ids, traj_id, X, y, alphas, reg, beta_a, beta_b):
    """
    ...
    for boot_id in batch_ids:
        results.append(
            run_bootstrap_trajectory(base_seed, traj_id, boot_id, X, y, alphas, reg, beta_a, beta_b)
        )
    return results
...
def run_bootstrap_for_all_trajectories(base_seed,
    ...
    with ProcessPoolExecutor(max_workers=num_workers) as exe:
        for traj in tqdm(range(n_trajectories), desc="Trajectories"):
            ...
            futures = {
                exe.submit(run_batch_trajectories, base_seed, batch_ids, traj, X, y, alphas, reg, beta_a, beta_b): batch_ids
